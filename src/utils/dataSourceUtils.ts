 ...
 * 获取数据源状态
 */
export const getDataSourceStatus = (source: DataSourceMetadataWithID): {
 ...
 * 验证数据源名称
 */
export const validateDataSourceName = (name: string): {
  valid: boolean
  message?: string
} => {
...
export const exportDataSourceConfig = (sources: DataSourceMetadataWithID[]): string => {
  const config = {
    ...
    sources: sources.map(source => ({
      ...
      type: source.source_type,      metadata: {
...
export const searchDataSources = (
  ...
  return sources.filter(source => {
    ...
    const matchDescription = searchDescription && source.description?.toLowerCase().includes(searchTerm)
