...
import type { FileItem, ColumnInfo, UploadResult, Dataset } from '@/types';
...
const uploadHistory = ref<Dataset[]>([])

// 计算属性
const columnInfo = computed<ColumnInfo[]>(() => {
...
// 方法
const handleFileChange = (file: FileItem): void => {
...
const beforeRemove = (): Promise<boolean> => {
  return ElMessageBox.confirm('确定移除该文件？')
    .then(() => true)
    .catch(() => false)
}
...
const useDataset = (dataset: Dataset): void => {
  appStore.setCurrentDataset(dataset as any)
  ElMessage.success('已切换到该数据集')
}

const deleteDataset = async (dataset: Dataset): Promise<void> => {
  try {
  ...
  <div class="data-upload">
    <div class="analysis-card">
      ...
      <el-upload ref="uploadRef" class="upload-demo" drag :auto-upload="false" :on-change="handleFileChange"
        :before-remove="beforeRemove" accept=".csv,.xlsx,.xls" :limit="1" :on-exceed="handleExceed">
      ...
      <div style="margin-top: 20px; text-align: center;">
        <el-button type="primary" @click="submitUpload" :loading="appStore.loading" :disabled="!selectedFile">
          <el-icon>
            <Upload />
          </el-icon>
    ...
    <div class="analysis-card" v-if="uploadResult">
      ...
      <div>
        ...
        <el-table :data="uploadResult.overview.head" style="width: 100%" max-height="400" border>
          <el-table-column v-for="column in uploadResult.overview.columns" :key="column" :prop="column" :label="column"
      ...
      <el-table :data="uploadHistory" style="width: 100%">
        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            <el-button size="small" @click="useDataset(scope.row)">
              使用此数据集
            </el-button>
            <el-button size="small" type="danger" @click="deleteDataset(scope.row)">
              删除
            </el-button>
          </template>
        </el-table-column>
