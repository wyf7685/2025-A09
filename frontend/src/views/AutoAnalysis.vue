    ...
    <div class="analysis-card">
      ...
      <div style="margin-bottom: 24px;">
        <el-alert
          title="自动分析说明"
          type="info"
          description="自动分析功能将对您的数据集进行全面的统计分析，包括数据概览、统计特征、相关性分析、异常检测等，并生成完整的分析报告。"
          show-icon
          :closable="false"
      ...
      <div class="report-content">
        ...
        <div v-if="analysisResult.figures && analysisResult.figures.length > 0" class="figures-section">
          ...
          <el-row :gutter="16">
            <el-col 
              :span="12" 
              v-for="(figure, index) in analysisResult.figures" 
              :key="index"
...
const runAnalysis = async () => {
  ...
  try {
    ...
    analysisHistory.value.unshift({
      ...
      dataset_shape: currentDataset.value.overview?.shape
    })
...
const deleteAnalysis = async (analysis, index) => {
  try {
...
const exportReport = async (format) => {
  ...
  try {
    await appStore.exportReport(appStore.currentSessionId, format)
