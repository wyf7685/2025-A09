    ...
    <div class="chat-container">
          ...
          <div class="message-content">
            ...
            <div v-else class="assistant-message">
              ...
              <div v-if="message.execution_results" class="execution-results">
                <el-collapse>
                  <el-collapse-item
                    v-for="(result, resultIndex) in message.execution_results"
                    :key="resultIndex"
      ...
      <div class="chat-input">
        ...
        <el-input
          ...
          type="textarea"
          :rows="3"
          placeholder="输入您想了解的数据分析问题，例如：分析数据的基本统计信息、查找异常值、绘制相关性热力图等..."
...
const sendMessage = async () => {
  ...
  try {
    ...
    const assistantMessage = {
      type: 'assistant',
      content: response.response || response.message || '抱歉，我无法处理您的请求。',
...
const loadChatHistory = () => {
  // 从 store 中加载聊天历史
  const history = appStore.chatHistory
  messages.value = history.map(entry => [
    ...
    },
    {
      ...
      charts: entry.execution_results?.filter(r => r.figure).map(r => r.figure) || []
    }
  ]).flat()
