  ...
  <div class="reports-page">
      ...
      <div v-else-if="!loading" class="reports-list">
        ...
        >
          ...
          <div class="report-preview">
            ...
            <div v-if="report.figures.length > 0" class="report-charts">
              <el-image
                v-for="(figure, index) in report.figures.slice(0, 3)"
                :key="index"
    ...
    >
      <div v-if="selectedReport" class="report-detail">
        ...
        <div v-if="selectedReport.figures.length > 0" class="report-figures">
          <h4>分析图表</h4>
          <div class="figures-grid">
            <el-image
              v-for="(figure, index) in selectedReport.figures"
              :key="index"
              :src="`data:image/png;base64,${figure.data}`"
              class="figure-image"
              fit="contain"
...
const refreshReports = async () => {
  ...
  try {
    ...
    if (session && session.analysis_results) {
      reports.value = session.analysis_results
    }
  } catch (error) {
...
.header-content h1 {
  margin: 0 0 8px 0;
  font-size: 32px;
  font-weight: 600;  background: linear-gradient(135deg, #3b82f6 0%, #6366f1 100%);
