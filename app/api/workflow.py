...
async def execute_workflow(request: ExecuteWorkflowRequest) -> dict:
    ...
    try:
        ...
        # 查找会话中的所有Dremio数据源
        dremio_sources = [ds_id for ds_id in session.dataset_ids if ds_id.startswith("dremio_")]
        ...
        if len(sources_dict) == 0:
            ...
            try:
                if settings.DREMIO_EXTERNAL_DIR.exists():
                    # 查找文件夹中的CSV文件
                    for file_path in settings.DREMIO_EXTERNAL_DIR.glob("*.csv"):
                        try:
                            # 创建数据源
                            file_uuid = file_path.stem
                            source = create_file_source(file_path, name=file_path.name)
