...
from app.core.agent.schemas import AgentRuntimeContext, DatasetID, OperationFailed
...
def get_sources() -> Sources:
    return AgentRuntimeContext.get().sources
...
def inspect_dataframe_tool(
    ...
    Returns:
        dict: 包含数据框详细信息的结果
    """
    return inspect_dataframe(get_sources().read(dataset_id), options)
...
def get_missing_values_summary_tool(dataset_id: DatasetID) -> MissingValuesSummary:
    ...
    Returns:
        缺失值摘要字典
    """
    return get_missing_values_summary(get_sources().read(dataset_id))
...
def handle_missing_values_tool(
    ...
    Returns:
        处理结果字典
    """
    return handle_missing_values(get_sources(), dataset_id, column, method)
...
def correlation_analysis_tool(
    ...
    return corr_analys(get_sources().read(dataset_id), col1, col2, method)
...
def detect_outliers_tool(
    ...
    """
    logger.opt(colors=True).info(
        f"<g>检测异常值</>: 列 <y>{escape_tag(column)}</>, 方法: <y>{escape_tag(method)}</>, 阈值: <y>{threshold}</>"
    )
    return detect_outliers(get_sources().read(dataset_id), column, method, threshold)
...
def lag_analysis_tool(
    ...
    """
    logger.opt(colors=True).info(f"执行<g>时滞分析</>: <y>{escape_tag(time_col1)}</> 与 <y>{escape_tag(time_col2)}</>")
    return lag_analys(get_sources().read(dataset_id), time_col1, time_col2)
...
def create_column_tool(
    ...
    return create_column(get_sources(), source_datasets, target_dataset_id, column_name, expression, description)
...
def create_interaction_term_tool(
    ...
    return create_interaction_term(
        get_sources().read(dataset_id), column_name, columns_to_interact, interaction_type, scale
    )
...
def create_aggregated_feature_tool(
    ...
    return create_aggregated_feature(
        get_sources().read(dataset_id), column_name, group_by_column, target_column, aggregation, description
    )
...
def join_dataframes_tool(
    ...
    return join_dataframes(
        get_sources(), left_dataset_id, right_dataset_id, join_type, left_on, right_on, new_dataset_id
    )
...
def combine_dataframes_tool(
    ...
    Returns:
        dict: 包含连接结果的字典。
    """
    return combine_dataframes(get_sources(), dataset_ids, operation, match_columns, ignore_index, new_dataset_id)
...
def create_dataset_from_query_tool(
    ...
    """
    return create_dataset_from_query(get_sources(), dataset_id, query, columns, reset_index, new_dataset_id)
...
def create_dataset_by_sampling_tool(
    ...
    Returns:
        dict: 包含新数据集ID和样本数据的结果字典。
    """
    return create_dataset_by_sampling(get_sources(), dataset_id, n, frac, random_state, stratify_by, new_dataset_id)
