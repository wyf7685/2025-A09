...
def handle_missing_values(df: pd.DataFrame, column: Optional[str] = None, method: str = 'drop') -> Dict[str, Any]:
    """
    处理数据框中的缺失值
    ...
    Args:
        ...
        method: 处理方法 ('drop', 'fill_mean', 'fill_median', 'fill_mode', 'fill_forward', 'fill_backward', 'interpolate')
...
def get_missing_values_summary(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"获取缺失值摘要失败: {str(e)}")
        return {
            'error': str(e),
            'total_missing': 0,
            'column_details': {}
