...
from app.core.agent.schemas import AgentRuntimeContext, DatasetID
...
def _cache_model_info(model_info: ModelInstanceInfo) -> ModelID:
    ...
    get_context().model_instance_cache[model_id] = model_info
    return model_id
...
def create_model_tool(
    ...
    if isinstance(hyperparams, str):
        ...
        if parsed_hyperparams is None:
            logger.opt(colors=True).warning(f"<y>超参数解析错误</y>: {escape_tag(repr(hyperparams))}，将使用默认参数")
    else:
...
def fit_model_tool(
    ...
    try:
        ...
        result = fit_model(
            context.sources.read(dataset_id),
            features,
            target,
            model,
            model_type,
...
def create_composite_model_tool(
    ...
    for model_id in model_ids:
        if model_id not in context.train_model_cache:
            raise ValueError(f"未找到训练好的模型 ID '{model_id}'")
        models.append(context.train_model_cache[model_id])
...
def evaluate_model_tool(trained_model_id: ModelID) -> EvaluateModelResult:
    ...
    if trained_model_id not in context.train_model_cache:
    ...
    return evaluate_model(context.train_model_cache[trained_model_id])
...
def save_model_tool(model_id: ModelID, model_name: str) -> SaveModelResult:
    ...
    result = save_model(context.train_model_cache[model_id], file_path)
    context.saved_models[model_id] = file_path

    # 注册模型到模型管理系统
    train_result = context.train_model_cache[model_id]
    # 计算模型性能指标
    ...
    session_name = ""
    if context.session_id and context.session_id in session_service.sessions:
        session_name = session_service.sessions[context.session_id].name or ""
    ...
    if dataset_id and context.sources.exists(dataset_id):
        datasource = context.sources.get(dataset_id)
        # 获取数据集的实际名称和描述
                ...
                if datasource_service.sources:
                    ...
                    for source_id, source in datasource_service.sources.items():
                        if (
                            ...
                            logger.opt(colors=True).info(f"使用原始数据源信息: 名称=<c>{escape_tag(dataset_name)}</c>")
                            break
            except Exception:
                logger.opt(colors=True).warning("获取原始数据源信息失败，使用当前数据源信息")

        logger.opt(colors=True).info(
            f"数据集信息: 名称=<c>{escape_tag(dataset_name)}</c>, 描述=<c>{escape_tag(dataset_description or '无')}</c>"
        )
...
def load_model_tool(dataset_id: DatasetID, model_id: ModelID) -> ModelMetadata:
    ...
    if model_id not in context.saved_models:
        raise ValueError(f"未找到保存的模型 ID '{model_id}'")

    file_path = context.saved_models[model_id]
    logger.opt(colors=True).info(f"<g>加载模型</>: <c>{escape_tag(str(file_path))}</>")
    metadata, train_result = load_model(context.sources.read(dataset_id), file_path)
    context.train_model_cache[model_id] = train_result
    return metadata
...
def list_saved_models_tool(include_external: bool = True) -> dict[ModelID, ModelMetadata]:
    ...
    # 获取当前会话已保存的模型
    models_dict = {model_id: load_model_metadata(file_path) for model_id, file_path in context.saved_models.items()}
...
def predict_with_model_tool(
    ...
    prediction, result = predict_with_model(
        context.train_model_cache[model_id],
        context.sources.read(dataset_id),
        input_features,
    )
    result["prediction_dataset_id"] = context.sources.create(
...
def select_features_tool(
    ...
    result, figure = select_features(
        context.sources.read(dataset_id), features, target, method, task_type, n_features, threshold
    )
    artifact = {}
...
def analyze_feature_importance_tool(
    ...
    result, figure = analyze_feature_importance(context.sources.read(dataset_id), features, target, method, task_type)
    artifact = {}
    if figure is not None:
        artifact = {"type": "image", "base64_data": base64.b64encode(figure).decode()}
    return result, artifact
...
def plot_learning_curve_tool(
    ...
    result, figure = plot_learning_curve(
        context.sources.read(dataset_id), features, target, model_type, task_type, cv_folds, scoring, None, hyperparams
    )
...
def check_model_compatibility_tool(dataset_id: DatasetID, model_id: ModelID) -> dict[str, Any]:
    ...
    # 加载模型元数据
    model_metadata = load_model_metadata(context.saved_models[model_id])
    df = context.sources.read(dataset_id)
