...
class CleanDataAgent:
    ...
    def _check_column_names(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        for col in df.columns:
            ...
            if col == "" or col.isspace():
                issues.append({"type": "column_name", "column": col, "description": "发现空列名"})

        # 检查重复列名
        issues.extend(
            {"type": "column_name", "column": col, "description": f'列名 "{col}" 重复出现'}
            for col in df.columns[df.columns.duplicated()].tolist()
        )
            ...
            if df[col].dtype == "object":
                ...
                if len(non_null_values) > 0:
                    ...
                    numeric_pattern_count = sum(
                        1 for val in sample_values if self.patterns["numeric"].match(str(val).strip())
                    )
        ...
        for col in df.columns:
            if df[col].dtype == "object":
                ...
                if any("email" in col.lower() or "@" in str(val) for val in sample_values):
                    invalid_emails = [
                        val for val in sample_values if "@" in str(val) and not self.patterns["email"].match(str(val))
                    ]
                    if invalid_emails:
                        issues.append(
                            {
    ...
    def _check_data_integrity(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        # 检查空行
        empty_rows = df.isna().all(axis=1).sum()  # pyright: ignore[reportAttributeAccessIssue]
        if empty_rows > 0:
            issues.append(
                {
                    "type": "data_integrity",
    ...
    def _calculate_quality_score(self, issues: list[dict[str, Any]], rows: int, columns: int) -> float:  # noqa: ARG002
        """计算质量分数"""
        base_score = 100.0
